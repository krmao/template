...
object HKToastUtil {
    ...
    private val DP_5 = HKSystemUtil.getPxFromDp(5f).toInt()
    ...
    fun show(msg: String) {
        if (!TextUtils.isEmpty(msg)) {
            if (toast == null) {
                toast = Toast.makeText(HKBaseApplication.INSTANCE, msg, Toast.LENGTH_SHORT)
    ...
    fun showDefault(msg: String) {
        if (!TextUtils.isEmpty(msg))
            Toast.makeText(HKBaseApplication.INSTANCE, msg, Toast.LENGTH_SHORT).show()
    }
    ...
    fun showMore(message: String) {
        if (TextUtils.isEmpty(message))
            return
        val toast = Toast(HKBaseApplication.INSTANCE)
        ...
        if (viewHolder != null) {
            ...
            val averageDip = HKSystemUtil.screenHeight / 7
            when (mToastCount) {
    ...
    private fun computeShowTime(tv: TextView, msg: String): Int {
        ...
        val maxWidth = (HKSystemUtil.screenWidth - HKSystemUtil.getPxFromDp(50f)).toInt()
    ...
    private val toastLayout: ViewHolder?
        get() {
            val contentLayout = LinearLayout(HKBaseApplication.INSTANCE)
            contentLayout.setBackgroundResource(R.drawable.hk_toast)
            val textView = TextView(HKBaseApplication.INSTANCE)
            textView.setShadowLayer(2.75f, 1f, 1f, Color.parseColor("#BB000000"))
            textView.setTextSize(TypedValue.COMPLEX_UNIT_SP, 18f)
            @Suppress("DEPRECATION")
            textView.setTextColor(HKBaseApplication.INSTANCE.resources.getColor(android.R.color.background_light))
