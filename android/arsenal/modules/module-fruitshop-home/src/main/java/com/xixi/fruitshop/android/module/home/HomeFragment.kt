...
class HomeFragment : FSBaseFragment(), FSBaseFragment.OnBackPressedListener {
    ...
    override fun onViewCreated(view: View?, savedInstanceState: Bundle?) {
            ...
            override fun onTabSelected(tab: TabLayout.Tab?) {
                when (tab?.position) {
                    0 -> {
                        ...
                        web_view.loadUrl(HomeActivity.HOME_URL)
                    }
                    1 -> {
                        web_view.clearHistory()
                        web_view.loadUrl(HomeActivity.HOME_CATEGORY_URL)
                    }
                    2 -> {
                        web_view.clearHistory()
                        web_view.loadUrl(HomeActivity.HOME_SEARCH_URL)
                    }
                    3 -> {
                        web_view.clearHistory()
                        web_view.loadUrl(HomeActivity.HOME_CART_URL)
                    }
                    4 -> {
                        web_view.clearHistory()
                        web_view.loadUrl(HomeActivity.HOME_USER_URL)
                    }
                }
            }
        })
        ...
        web_view.setWebViewClient(object : FSWebViewUtil.FSWebViewClient() {
            ...
            override fun shouldOverrideUrlLoading(_view: WebView?, _url: String?): Boolean {
                ...
                when (_url) {
                    HomeActivity.HOME_URL -> {
                        clearHistory()
                        tab_layout.getTabAt(0)?.select()
                    }
                    HomeActivity.HOME_CATEGORY_URL -> {
                        clearHistory()
                        tab_layout.getTabAt(1)?.select()
                    }
                    HomeActivity.HOME_SEARCH_URL -> {
                        clearHistory()
                        tab_layout.getTabAt(2)?.select()
                    }
                    HomeActivity.HOME_CART_URL -> {
                        clearHistory()
                        tab_layout.getTabAt(3)?.select()
                    }
                    HomeActivity.HOME_USER_URL -> {
                        clearHistory()
            ...
            override fun onPageCommitVisible(_view: WebView?, _url: String?) {
                FSLogUtil.d("krmao", "onPageCommitVisible:" + _url)
                super.onPageCommitVisible(_view, _url)
            }

            override fun onLoadResource(_view: WebView?, _url: String?) {
                //FSLogUtil.d("krmao", "onLoadResource:" + _url)
                super.onLoadResource(_view, _url)
            }
            ...
            override fun onPageFinished(webView: WebView?, url: String?) {
                ...
                //loading_view.showView(FSFrameLoadingLayout.ViewType.NODATA)
            }
        })
        ...
        web_view.setWebChromeClient(object : FSWebViewUtil.FSWebChromeClient() {
            override fun onProgressChanged(_view: WebView, newProgress: Int) {
                ...
                //loading_view.updateText(FSFrameLoadingLayout.ViewType.LOADING, "加载进度:" + newProgress, true, true)
            }
