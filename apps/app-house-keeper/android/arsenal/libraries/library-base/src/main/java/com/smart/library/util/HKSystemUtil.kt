...
object HKSystemUtil {
    ...
    val displayMetrics: DisplayMetrics
        get() = HKBaseApplication.INSTANCE.resources.displayMetrics

    val screenWidth: Int
        get() = HKBaseApplication.INSTANCE.resources.displayMetrics.widthPixels
    ...
    val statusBarHeight: Int
        get() {
            ...
            if (resourceId > 0) {
                statusBarHeight = HKBaseApplication.INSTANCE.resources.getDimensionPixelSize(resourceId)
            }
    ...
    val versionCode: Int
        get() {
            var versionCode = 0
            try {
                versionCode = HKBaseApplication.INSTANCE.packageManager.getPackageInfo(HKBaseApplication.INSTANCE.packageName, 0).versionCode
    ...
    val versionName: String?
        get() {
            ...
            try {
                versionName = HKBaseApplication.INSTANCE.packageManager.getPackageInfo(HKBaseApplication.INSTANCE.packageName, 0).versionName
            } catch (e: PackageManager.NameNotFoundException) {
                e.printStackTrace()
            }
            return versionName
    ...
    fun getAppMetaData(key: String): Any? {
        ...
        try {
            val info = HKBaseApplication.INSTANCE.packageManager.getApplicationInfo(HKBaseApplication.INSTANCE.packageName, PackageManager.GET_META_DATA)
