...
class HKNetworkChangedReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (WifiManager.WIFI_STATE_CHANGED_ACTION == intent.action) {
            ...
            when (wifiState) {
                WifiManager.WIFI_STATE_DISABLED -> if (mLastNetworkType == null)
                    HKLogUtil.w(TAG, "首次启动，多多关照....")
                else
                    HKLogUtil.e(TAG, ">>>>--------start-------->>>>")
        ...
        /*if (WifiManager.NETWORK_STATE_CHANGED_ACTION.equals(intent.getAction())) {
            ...
            if (null != parcelableExtra) {
                ...
                HKLogUtil.w(TAG, "网络状态变化广播  WIFI: " + wifi.isConnected() + "- GPRS: " + gprs.isConnected() + "- 当前状态: " + MNetworkUtil.getNetType(manager));
            }
        }*/
        ...
        if (ConnectivityManager.CONNECTIVITY_ACTION == intent.action) {
            ...
            //val gprs = manager.getNetworkInfo(ConnectivityManager.TYPE_MOBILE)
            //val wifi = manager.getNetworkInfo(ConnectivityManager.TYPE_WIFI)
            val currentNetworkType = HKNetworkUtil.getNetType(manager)
            if (mLastNetworkType != null) {//第一次
                if (mLastNetworkType != currentNetworkType) {
                    // todo others
                    HKLogUtil.w(TAG, "呼叫下载处理逻辑")
                    //
                    HKLogUtil.e(TAG, "<<<<--------end--------<<<<")
    ...
    companion object {
        ...
        fun register(context: Context): HKNetworkChangedReceiver {
            ...
            val networkChangedReceiver = HKNetworkChangedReceiver()
