 ...
 */
class CXBaseBundleHelper(info: CXBundleInfo, rootDir: File, val pathInAssets: String) : CXIBundleHelper(info, rootDir) {

    fun getBaseDir(): File = CXCacheManager.getChildDir(rootDir, CXDeployConstants.DIR_NAME_BASE.md5(CXDeployManager.debug))
    fun getBaseZipFile(): File = File(getBaseDir(), String.format(CXDeployConstants.FILE_NAME_BASE_ZIP, info.version).md5(CXDeployManager.debug))
