...
object CXIntentUtil {
    ...
    fun goToAppDetails(context: Context?) = goToDefault(context, getInstalledAppDetailsIntent(CXBaseApplication.INSTANCE.packageName))
    ...
    fun callOpenInstallApk(context: Context, apkFilePath: String) {
        val intent = Intent(Intent.ACTION_VIEW)
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) intent.flags = Intent.FLAG_GRANT_READ_URI_PERMISSION
        intent.setDataAndType(CXUriUtil.fromFileProvider(apkFilePath), "application/vnd.android.package-archive")
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
        context.startActivity(intent)
    }
