...
object CXHybirdBridge {
    ...
    fun callNativeMethod(className: String?, methodName: String?, vararg params: Any?): Any? {
        ...
        return CXReflectUtil.invoke(classMap[className], methodName, params)
    }
    ...
    fun addNativeClass(scheme: String, className: String, kClass: KClass<*>?) {
        ...
        schemeMap.put(scheme, { webView: WebView?, _: WebViewClient?, schemeUrlString: String?, _: (() -> Unit?)? ->
            ...
            if (pathSegments.size >= 2) {
                ...
                CXLogUtil.d(TAG, "clazzName:$clazzName , methodName:$methodName , params:size:${paramArray?.size}:($params) , hashCode:$hashcode")
                CXLogUtil.w(TAG, "native.invoke start --> [${kClass.java.name}.$methodName($params)]")

                var result: Any? = null
                try {
                    result = callNativeMethod(className, methodName, *paramArray ?: arrayOf())
                } catch (e: Exception) {
                    CXLogUtil.e(TAG, "native.invoke exception", e)
                }

                CXLogUtil.w(TAG, "native.invoke end , result:$result")
                if (!TextUtils.isEmpty(hashcode)) {
                    callJsFunction(webView, "javascript:window.hybird.onCallback($hashcode, $result)")
                }
                true
            } else {
                CXLogUtil.e(TAG, "schemaUrl:$schemeUrl 格式定义错误，请参照 hybird://native/className/methodName?params=1,2,3,4,5&hashcode=123445")
                false
            }
        })
    }
    ...
    fun shouldOverrideUrlLoading(webView: WebView?, webViewClient: WebViewClient?, uriString: String?, callback: (() -> Unit?)? = null): Boolean {
        ...
        for (entry in list.sortedByDescending { it.key.length }) {
            CXLogUtil.v(TAG, "shouldOverrideUrlLoading [检测开始] 检测本URL是否被拦截, 匹配: ${entry.key}")
            val intercept = entry.value.invoke(webView, webViewClient, uriString, callback)
            CXLogUtil.v(TAG, "shouldOverrideUrlLoading [检测结束] 本URL ${if (intercept) "被拦截 " else "未被拦截 "}")
            if (intercept) {
                return true
            } else {
                continue
...
//
//        CXLogUtil.d(TAG, "get uri : $uri")
//        CXLogUtil.d(TAG, "get schemePrefix : $schemePrefix")
//        CXLogUtil.d(TAG, "do intercept ? ${schemeMap.containsKey(schemePrefix)}")
//
...
//    }
    ...
    fun addSchemeForCallback() = addScheme(callbackSchemaPrefix) { _: WebView?, _: WebViewClient?, urlString: String?, _: (() -> Unit?)? ->
        ...
        CXLogUtil.e(TAG, "callbackMap.size[after callback]:" + callbackMap.size)
        true
    }
    ...
    init {
        addNativeClass("hybird://hybird:1234", "native", CXHybirdMethods::class)
    }
}
