...
object CXNavigationManager {
    ...
    @JvmStatic
    var lastSelectedMapType: MapType = MapType.valueOf(CXPreferencesUtil.getString(KEY_LAST_SELECTED_MAP_TYPE, MapType.AMAP.name))
        private set(value) {
            field = value
            CXPreferencesUtil.putString(KEY_LAST_SELECTED_MAP_TYPE, value.name)
        }
    ...
    enum class MapType(val displayName: String, val packageName: String, val mapSupportScope: MapSupportScope) {
        ...
        fun isAppInstalled(): Boolean = CXSystemUtil.isAppInstalled(packageName)
            ...
            when (this) {
                CXNavigationManager.MapType.AMAP -> {
                    try {
                        ...
                        intentUri.append("?sourceApplication=${CXSystemUtil.appName}&slat=$fromGCJ02Lat&slon=$fromGCJ02Lon&sname=${if (TextUtils.isEmpty(fromAddressName)) "起点" else fromAddressName}\"")
                        ...
                        CXLogUtil.e("start auto naviMap exception:$e")
                        CXToastUtil.show("您当前地图版本可能不支持导航功能")
                    }
                }
                ...
                CXNavigationManager.MapType.GOOGLE -> {
                    try {
                        ...
                        CXLogUtil.e("start google map exception:$e")
                        CXToastUtil.show("您当前地图版本可能不支持导航功能")
                    }
                }
            }
