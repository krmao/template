 ...
 *          app:cxEmptyView="@layout/cx_widget_frameloading_empty"      //自定义布局 -> 没有数据
 *          app:cxFailureView="@layout/cx_widget_frameloading_failure"  //自定义布局 -> 网络异常
 *          app:cxLoadingView="@layout/cx_widget_frameloading_loading"  //自定义布局 -> 正在加载
...
class CXFrameLoadingLayout : FrameLayout {
    ...
    companion object {
        var LAYOUT_ID_LOADING = R.layout.cx_widget_frameloading_loading
        var LAYOUT_ID_NO_DATA = R.layout.cx_widget_frameloading_empty
        var LAYOUT_ID_NET_WORK_EXCEPTION = R.layout.cx_widget_frameloading_failure
    }
    ...
    constructor(context: Context, attrs: AttributeSet?, defStyle: Int) : super(context, attrs, defStyle) {
        if (attrs != null) {
            ...
            if (typedArray != null) {
                val loadingLayoutId = typedArray.getResourceId(R.styleable.CXFrameLoadingLayout_cxLoadingView, LAYOUT_ID_LOADING)
                val networkExceptionLayoutId = typedArray.getResourceId(R.styleable.CXFrameLoadingLayout_cxFailureView, LAYOUT_ID_NET_WORK_EXCEPTION)
                val nodataLayoutId = typedArray.getResourceId(R.styleable.CXFrameLoadingLayout_cxEmptyView, LAYOUT_ID_NO_DATA)
                val centerInParent = CXCustomViewUtil.getBoolean(typedArray, R.styleable.CXFrameLoadingLayout_cxCenterInParent, false)
                val useSmallStyle = CXCustomViewUtil.getBoolean(typedArray, R.styleable.CXFrameLoadingLayout_cxUseSmallStyle, false)
                val defaultScaleFactor = 0.6f
                var scaleFactor = CXCustomViewUtil.getFloat(typedArray, R.styleable.CXFrameLoadingLayout_cxScaleFactor, defaultScaleFactor)
                if (scaleFactor <= 0 || scaleFactor > 1)
                    scaleFactor = defaultScaleFactor
                val drawable = CXCustomViewUtil.getDrawable(typedArray, R.styleable.CXFrameLoadingLayout_cxAllBackground, Color.parseColor("#FFFFFFFE"), -1)
                typedArray.recycle()
    ...
    fun getDefaultText(viewType: ViewType): String {
        return when (viewType) {
            ViewType.NODATA -> resources.getString(R.string.cx_frameloading_empty)
            ViewType.LOADING -> resources.getString(R.string.cx_frameloading_loadingnow)
            ViewType.NETWORK_EXCEPTION -> resources.getString(R.string.cx_frameloading_networkerror)
        }
    }
