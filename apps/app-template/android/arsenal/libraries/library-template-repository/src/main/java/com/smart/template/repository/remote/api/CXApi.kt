package com.smart.template.repository.remote.api
...
interface CXApi {
    ...
    @GET
    fun downloadHybirdModuleConfiguration(@Url url: String): Observable<CXHybirdModuleConfigModel>

    @GET
    fun downloadHybirdAllModuleConfigurations(@Url url: String): Observable<MutableList<CXHybirdModuleConfigModel>>
     ...
     * 上传, 例:
     *
     *      CXApiManager.getApi(CXApi.class)
     *              .upload(url, CXApi.Companion.createBody(file, new ProgressListener() {...}))
     *              .subscribe(...);
     *
     ...
     */
    @POST
    fun upload(@Url url: String, @Body body: RequestBody): Observable<CXResponseModel<Any>>
