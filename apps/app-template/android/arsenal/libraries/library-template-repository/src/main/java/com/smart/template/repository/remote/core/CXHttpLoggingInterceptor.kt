 ...
 */
class CXHttpLoggingInterceptor(private var level: Level = Level.NONE, private val logger: Logger = Logger.DEFAULT) : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        logger.log(("""<-- ${response.code()} [RAW:${String.format("%4d", response.networkResponse()?.code())} (${CXTimeUtil.Hms()})] ${response.message()} ${response.request().url()} (${tookMs}ms${if (!logHeaders) ", $bodySize body" else ""})"""))
