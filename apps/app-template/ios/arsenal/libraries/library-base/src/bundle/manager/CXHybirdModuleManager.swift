...
class CXHybirdModuleManager{
    ...
    init(config: CXHybirdModuleConfigModel) {
        this.currentConfig = config//手动重新赋值一下才会调用 其自定义的 set 方法, 从而设置拦截器
    }

    static func create(config: CXHybirdModuleConfigModel?) -> CXHybirdModuleManager? {
        if (config == nil || !CXHybirdUtil.isLocalFilesValid(config)) {
            CXLogUtil.e(CXHybird.TAG + ":" + config?.moduleName, ">>>>[错误]<<<< 本地解压文件夹校验失败,不能初始化模块 return nil !!!")
            return nil
        } else {
