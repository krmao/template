...
class CXWKWebViewURLProtocol: URLProtocol {
    ...
    private static let KEY = "CXWKWebViewURLProtocol_REQUEST_KEY"
    ...
    override class func canInit(with request: URLRequest) -> Bool {
        ...
        var canInit = request.url?.host == CXWKWebViewURLProtocol.HOST

        if !canInit {
            return false
        }

        canInit = canInit && CXWKWebViewURLProtocol.isLocalFileExists(request)

        if !canInit {
            CXLogUtil.d(CXWKWebViewURLProtocol.TAG, "canInit:false, host:true, isLocalFileExists:false url:" + (request.url?.absoluteString ?? ""))
            return false
        }

        if (URLProtocol.property(forKey: CXWKWebViewURLProtocol.KEY, in: request) != nil) {
            canInit = false
            CXLogUtil.d(CXWKWebViewURLProtocol.TAG, "canInit:false, host:true, isLocalFileExists:true, repeatRequest url:" + (request.url?.absoluteString ?? ""))
            return canInit
        }

        CXLogUtil.d(CXWKWebViewURLProtocol.TAG, "canInit:false, host:true, isLocalFileExists:true url:" + (request.url?.absoluteString ?? ""))
        return canInit
    }
    ...
    class func isLocalFileExists(_ request: URLRequest) -> Bool {
        return CXFileUtil.fileExists(CXWKWebViewURLProtocol.getLocalFilePath(request))
    }
    ...
    override func startLoading() {
        CXLogUtil.d(CXWKWebViewURLProtocol.TAG, "startLoading originUrl:" + (self.request.url?.absoluteString ?? ""))

        URLProtocol.setProperty(true, forKey: CXWKWebViewURLProtocol.KEY, in: self.request as! NSMutableURLRequest)

        if self.request.url?.host == CXWKWebViewURLProtocol.HOST {
            ...
            let localFilePath = request.url?.absoluteString
                    .replace("https://" + CXWKWebViewURLProtocol.HOST, String(CXBundleManager.INSTANCE.pathForHybirdDir.dropLast(1)))
                    .replace("http://" + CXWKWebViewURLProtocol.HOST, String(CXBundleManager.INSTANCE.pathForHybirdDir.dropLast(1))) ?? ""
            //let localFilePath = String( CXBundleManager.INSTANCE.pathForHybirdDir.dropLast(1) + (self.request.url?.path ?? ""))
            CXLogUtil.d(CXWKWebViewURLProtocol.TAG, "startLoading localUrl:" + localFilePath)
