...
- (void)registSubscribModel:(STEventSubscribeModel *)subscrib forKey:(id)target{
    if(!subscrib) {return;}
    if(!target) {return;}

    switch (subscrib.priority) {
        case STEventSubscriberPriorityHigh:{
            [self addSubscribModel:subscrib forKey:target toMap:self.mapHigh];
        }
            break;
        case STEventSubscriberPriorityDefault:{
            [self addSubscribModel:subscrib forKey:target toMap:self.mapDefault];
        }
            break;
        case STEventSubscriberPriorityLow:{
...
- (void)postEventWithDeliveryData:(STEventUserInfo *)info isInMain:(BOOL)isMain{
#ifdef DEBUG
    self.triggerCount++;
#endif
...
- (void)addSubscribModel:(STEventSubscribeModel *)subscrib forKey:(id)target toMap:(NSMapTable *)map{
    [map setObject:subscrib forKey:target];
}

#pragma mark ======  c  ======
static dispatch_queue_t event_subscriber_dispatcher_serialQueue() {
    static dispatch_queue_t st_event_subscriber_dispatcher_serialQueue;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        st_event_subscriber_dispatcher_serialQueue = dispatch_queue_create("COM.ST_EVENT.SERIAL_QUEUE", DISPATCH_QUEUE_SERIAL);

        dispatch_queue_t referQueue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
        dispatch_set_target_queue(st_event_subscriber_dispatcher_serialQueue, referQueue);

    });
    return st_event_subscriber_dispatcher_serialQueue;
}
static dispatch_queue_t event_subscriber_dispatcher_concurrentQueue() {
    static dispatch_queue_t st_event_subscriber_dispatcher_concurrentQueue;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        st_event_subscriber_dispatcher_concurrentQueue = dispatch_queue_create("COM.ST_EVENT.CONCURRENT_QUEUE", DISPATCH_QUEUE_CONCURRENT);

        dispatch_queue_t referQueue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
        dispatch_set_target_queue(st_event_subscriber_dispatcher_concurrentQueue, referQueue);
    });
    return st_event_subscriber_dispatcher_concurrentQueue;
}
