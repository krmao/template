...
object STPreferencesUtil {
    ...
    private var mSharedPreferences: SharedPreferences? = STInitializer.application()?.getSharedPreferences(STConfig.NAME_SHARED_PREFERENCES, Context.MODE_PRIVATE)
    //private var objectMapper: ObjectMapper = ObjectMapper().registerKotlinModule()
    private var mGson: Gson = Gson()

    fun getBoolean(name: String, bDefault: Boolean): Boolean? = mSharedPreferences?.getBoolean(name, bDefault)
