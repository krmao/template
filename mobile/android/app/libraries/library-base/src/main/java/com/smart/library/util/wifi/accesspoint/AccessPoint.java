...
package com.smart.library.util.wifi.accesspoint;
...
public class AccessPoint implements Comparable<AccessPoint> {
    ...
    public boolean matches(WifiConfiguration config) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && config.isPasspoint() && mConfig != null && mConfig.isPasspoint()) {
            return ssid.equals(removeDoubleQuotes(config.SSID)) && config.FQDN.equals(mConfig.FQDN);
        } else {
            return ssid.equals(removeDoubleQuotes(config.SSID)) && security == getSecurity(config) && (mConfig == null/* || mConfig.shared == config.shared*/); // todo
        }
    }
    ...
    public void clearConfig() {
        mConfig = null;
        networkId = -1; //WifiConfiguration.INVALID_NETWORK_ID;
    }
    ...
    public boolean isMetered() {
        return mIsScoredNetworkMetered /*|| WifiConfiguration.isMetered(mConfig, mInfo)*/; // todo
    }
    ...
    public String getConfigName() {
        if (mConfig != null && Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && mConfig.isPasspoint()) {
...
*/
   /* private String getSettingsSummary(WifiConfiguration config) {
    ...
    public boolean isActive() {
        return mNetworkInfo != null && (networkId != -1 || mNetworkInfo.getState() != State.DISCONNECTED);
    }
    ...
    public boolean isEphemeral() {
        return mInfo != null /*&& mInfo.isEphemeral() */ && mNetworkInfo != null && mNetworkInfo.getState() != State.DISCONNECTED; // todo
    }
    ...
    public boolean isPasspoint() {
        return mConfig != null && (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M ? mConfig.isPasspoint() : true); // todo
    }
    ...
    private boolean isInfoForThisAccessPoint(WifiConfiguration config, WifiInfo info) {
        if (isPasspoint() == false && networkId != -1) {
    ...
    public void saveWifiState(Bundle savedState) {
        ...
        savedState.putParcelableArrayList(KEY_SCOREDNETWORKCACHE, (ArrayList<? extends Parcelable>) new ArrayList<>(mScoredNetworkCache.values()));
        if (mNetworkInfo != null) {
    ...
    void setScanResults(Collection<ScanResult> scanResults) {
        ...
        if (!scanResults.isEmpty()) {
            ...
            mCarrierName = result.carrierName;*/ // todo
        }
    }
    ...
    void update(@Nullable WifiConfiguration config) {
        ...
        STThreadUtils.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                if (mAccessPointListener != null) {
                    mAccessPointListener.onAccessPointChanged(AccessPoint.this);
    ...
    private static boolean isVerboseLoggingEnabled() {
        return /*WifiTracker.sVerboseLogging ||*/ Log.isLoggable(TAG, Log.VERBOSE); // todo
    }
}
