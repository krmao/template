...
public class STBottomSheetViewDragHelper {
    ...
    public static STBottomSheetViewDragHelper create(@NonNull final ViewGroup forParent, @NonNull final Callback cb) {
        return new STBottomSheetViewDragHelper(forParent.getContext(), forParent, cb);
    }

    public static STBottomSheetViewDragHelper create(@NonNull final ViewGroup forParent, final float sensitivity, @NonNull final Callback cb) {
        final STBottomSheetViewDragHelper helper = create(forParent, cb);
        helper.mTouchSlop *= (int) (1.0f / sensitivity);
        return helper;
    }

    private STBottomSheetViewDragHelper(@NonNull final Context context, @NonNull final ViewGroup forParent, @NonNull final Callback cb) {
        this.mActivePointerId = -1;
        this.mSetIdleRunnable = new Runnable() {
            @Override
            public void run() {
                STBottomSheetViewDragHelper.this.setDragState(0);
            }
        };
        if (forParent == null) {
        ...
        this.mScroller = new OverScroller(context, STBottomSheetViewDragHelper.sInterpolator);
    }
    ...
    public boolean continueSettling(final boolean deferCallbacks) {
        if (this.mDragState == 2) {
            ...
            final int dx = 0; //x - this.mCapturedView.getLeft();
            ...
            case 2: {
                ...
                for (int pointerCount = ev.getPointerCount(), i = 0; i < pointerCount; ++i) {
                    ...
                    if (this.isValidPointerForActionMove(pointerId)) {
                        ...
                        final float dx = 0;// x3 - this.mInitialMotionX[pointerId];
