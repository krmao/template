...
public class ColorPickerPalette extends TableLayout {
    ...
    public void init(int size, int columns, ColorPickerSwatch.OnColorSelectedListener listener) {
        ...
        if (size == ColorPickerDialog.SIZE_LARGE) {
            mSwatchLength = (int) STSystemUtil.getPxFromDp(64);
            mMarginSize = (int) STSystemUtil.getPxFromDp(8);
        } else {
            mSwatchLength = (int) STSystemUtil.getPxFromDp(48);
            mMarginSize = (int) STSystemUtil.getPxFromDp(4);
        }
        mOnColorSelectedListener = listener;

        mDescription = "色彩";
        mDescriptionSelected = "已选取色彩";
    }
