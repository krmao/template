 ...
 *
 *  海外 + 台湾 用 wgs84 坐标系比较精确
 *  大陆 + 港澳 用 gcj02 坐标系比较精确
 */
@Suppress("MemberVisibilityCanBePrivate", "unused")
data class STLatLng(val latitude: Double = 0.0, val longitude: Double = 0.0, val type: STLatLngType = STLatLngType.GCJ02) {
    ...
    companion object {
        ...
        @JvmStatic
        fun isValidLatLng(latitude: Double?, longitude: Double?) = if (latitude == null || longitude == null) false else (latitude != 0.0 && longitude != 0.0 && latitude != -1.0 && longitude != -1.0 && abs(latitude) <= 90 && abs(longitude) <= 180)
