...
public class KeyTimeCycle extends Key
{
    ...
    public void load(final Context context, final AttributeSet attrs) {
        final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.KeyTimeCycle);
        Loader.read(this, a);
    }
    ...
    public void addTimeValues(final HashMap<String, TimeCycleSplineSet> splines) {
        for (final String s : splines.keySet()) {
            final TimeCycleSplineSet splineSet = splines.get(s);
    ...
    private static class Loader
    {
        ...
        public static void read(final KeyTimeCycle c, final TypedArray a) {
            for (int N = a.getIndexCount(), i = 0; i < N; ++i) {
                ...
                switch (Loader.mAttrMap.get(attr)) {
                    ...
                    default: {
                        Log.e("KeyTimeCycle", "unused attribute 0x" + Integer.toHexString(attr) + "   " + Loader.mAttrMap.get(attr));
                        break;
                    }
                }
            }
