...
const {APP_DOMAIN_LOCALHOST_PORT, APP_DOMAIN_LOCALHOST, APP_DOMAIN_PROD, APP_REQUEST_PATH_PREFIX} = require('./src/basic/basic-constants')

nextApp.prepare().then(() => {                                          // 等待 nestjs 页面编译完成
    const koaServer = new koa()                                         // 创建一个 Koa 对象表示 web app 本身
    const koaServerRouter = new koaRouter()

    koaServer.use(async (context, next) => {                        // 对于任何请求，app将调用该异步函数处理请求
        // console.log("koaServer context:", context, "next:", next);
        if (context.url.startsWith(APP_REQUEST_PATH_PREFIX)) {
            context.respond = false
            await kos2Connect(createProxyMiddleware({target: APP_DOMAIN_PROD, changeOrigin: true, pathRewrite: {"^/api": ''}}))(context, next)   // 等待代理执行完成
        } else {
    ...
    koaServer.listen(APP_DOMAIN_LOCALHOST_PORT, () => {
        console.log(`> Ready on ${APP_DOMAIN_LOCALHOST}`)
    })
})
