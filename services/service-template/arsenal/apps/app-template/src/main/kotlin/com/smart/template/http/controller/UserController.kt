...
import com.smart.template.http.model.*
...
@RestController
@RequestMapping("/user")
class UserController(val userMapper: UserMapper, val roleMenuMapper: RoleMenuMapper, val menuMapper: MenuMapper, val roleMapper: RoleMapper) {
    private val logger: Logger = LogManager.getLogger(UserController::class.java.name)

    @ApiOperation("根据角色ID获取所有的菜单")
    @PostMapping("/findAllMenusByRoleId")
    fun findAllMenusByRoleId(@RequestBody request: HKRequest<HKRoleIdData>): HKResponse<List<MenuModel?>> {
        if (request.data == null || request.data?.roleId == null) return HKCode.ERROR_PARAMS.response(arrayListOf())

        return HKResponse(roleMenuMapper.select(RoleMenuModel(roleId = request.data?.roleId)).map { menuMapper.select(MenuModel(id = it.roleId)).firstOrNull() })
    }

    @ApiOperation("根据用户ID获取所有的菜单")
    @PostMapping("/findAllMenusByUserId")
    fun findAllMenusByUserId(@RequestBody request: HKRequest<HKUserIdData>): HKResponse<List<MenuModel?>> {
        if (request.data == null || request.data?.userId == null) return HKCode.ERROR_PARAMS.response(arrayListOf())

        return HKResponse(roleMenuMapper.select(RoleMenuModel(roleId = request.data?.userId)).map { menuMapper.select(MenuModel(id = it.menuId)).firstOrNull() })
    }

    @ApiOperation("根据菜单ID获取所有的角色")
    @PostMapping("/findAllRolesByMenuId")
    fun findAllRolesByMenuId(@RequestBody request: HKRequest<HKMenuIdData>): HKResponse<List<RoleModel?>> {
        if (request.data == null || request.data?.menuId == null) return HKCode.ERROR_PARAMS.response(arrayListOf())

        return HKResponse(roleMenuMapper.select(RoleMenuModel(menuId = request.data?.menuId)).map { roleMapper.select(RoleModel(it.roleId)).firstOrNull() })
    }
