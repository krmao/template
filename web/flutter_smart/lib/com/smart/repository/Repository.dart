...
class Repository {
  ...
  static Future<dynamic> requestLogin(String mobile, String password) async {
    ...
    _requestFormPost("/user/login", data: new FormData.from({"mobile": mobile, "password": password})).then((ResponseModel responseModel) {
  ...
  static Future<dynamic> requestAuth(int id, String surname, String name, String idcard, File image1, File image2, File image3) async {
    Completer<dynamic> completer = new Completer();
  ...
  static Future<dynamic> requestMyDeal(int id) async {
    Completer<dynamic> completer = new Completer();
    _requestFormPost("/user/my_deal", data: new FormData.from({"id": id})).then((ResponseModel responseModel) {
      completer.complete(responseModel.data);
    }).catchError((error) {
      completer.completeError(error);
    });
